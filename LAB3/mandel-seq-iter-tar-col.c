	...
	tareador_disable_object(&equal);
      for (int x = 0; x < COLS; x += TILE) {
	    ...
	    for (int y = 0; y < ROWS; y += TILE) {
		  ...
		  //tareador_end_task("mandel_H");
		  //check vertical borders
		  //tareador_start_task("mandel_V");
		...
		//tareador_end_task("mandel_V");
		  //check if we can avoid computation of tile
		  //tareador_start_task("mandel_IF");
		  if (equal) {
			if (output2histogram)
			...
			for (int py = y; py < y + TILE; py++) {
				//tareador_start_task("mandel_fill_all_py");
			      for (int px = x; px < x + TILE; px++) {
				    M[py][px] = M[y][x];
				    if (output2display) {
			      ...
			      //tareador_end_task("mandel_fill_all_py");
            }
		  } else {
			// computation of tile
			for (int py = y; py < y + TILE; py++) {
				//tareador_start_task("mandel_comp_py");
			      for (int px = x; px < x + TILE; px++) {
			      ...
			      //tareador_end_task("mandel_comp_py");
            }
		  }
		  //tareador_end_task("mandel_IF");
		  //tareador_end_task("mandelbrot");
	    }
	    tareador_end_task("Loop_X");
      }
}
...
int main(int argc, char *argv[])
{
      ...
      fprintf(stdout, "Mandelbro:t set: Computed\n");
      if (output2histogram)
