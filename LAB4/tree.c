...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      y = start_fil;
      x = start_col;
#pragma omp task shared(equal)	//check_H
{
	equal = non_first_call;
...
#pragma omp task shared(equalV)	//check_V
{
	equalV = non_first_call;
	for (int py = y; py < y + NRows; py++) {
	    ...
	    equalV = equalV & (M[y][x] == M[py][x + NCols - 1]);
      }
}
      //check if we can avoid computation of tile
//#pragma omp task depend(in:equal, equalV)
#pragma omp taskwait
	if (equal & equalV) {
...
//#pragma omp task firstprivate(y,x)
		...
		if(!omp_in_final()) {
		  if (NRows > TILE) {
#pragma omp task final(NCols < CUTOFF)
