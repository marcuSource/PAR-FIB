...
#pragma omp task
	...
	#pragma omp task depend(out: equal) //mandel_H
	{
			equal = 1;
	...
	#pragma omp task depend(out: equalV) //mandel_V
	{
			equalV = 1;
			for (int py = y; py < y + TILE; py++) {
			      ...
			      equalV = equalV & (M[y][x] == M[py][x]);
			      equalV = equalV & (M[y][x] == M[py][x + TILE - 1]);
			}

	}
	#pragma omp task depend(in: equal, equalV) //mandel_IF
	{
			if (equal & equalV) {
				...
				#pragma omp atomic
			      ...
			      for (int py = y; py < y + TILE; py++)
			      {
				#pragma omp task //mandel_fill_all_py
				{
				    for (int px = x; px < x + TILE; px++) {
					  M[py][px] = M[y][x];
					  if (output2display) {
			      ...
			      for (int py = y; py < y + TILE; py++)
			      {
				 #pragma omp task //mandel_comp_py
				 {
				    for (int px = x; px < x + TILE; px++) {
